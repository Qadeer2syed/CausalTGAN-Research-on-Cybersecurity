"""
Preprocess NSL-KDD Test Set (All 41 Features)
Matches the format of train_full.csv
"""
...
def preprocess_labels(data):
    ...
    if len(unmapped) > 0:
        ...
        unique_unmapped = data.loc[data['label'].isna(), 'label'].unique()
...
def save_data(data, output_dir):
    ...
    data.to_csv(test_path, index=True)
    print(f"\n[OK] Saved test dataset: {test_path}")
    print(f"     {len(data)} samples, {len(data.columns)} features")

    return test_path

def main():
    INPUT_FILE = r"KDDTest-21.txt"
    OUTPUT_DIR = r"C:\Users\qadee\Desktop\CausalTGAN\data\real_world\nsl_kdd"

    print("="*70)
    print("PREPROCESSING NSL-KDD TEST SET (ALL 41 FEATURES)")
    print("="*70)
    ...
    print(f"\nNext: Run evaluation with actual test set")
    print("  python evaluate_synthetic_data.py")
    print("="*70)
