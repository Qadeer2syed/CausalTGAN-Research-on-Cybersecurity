...
from model.causalTGAN import load_model
from helper.utils import restore_feature_info
...
if __name__ == '__main__':
    gen_num = 25000  # Generate 25k samples (same size as training data)
    device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
    model_path = 'Testing/CausalTGAN_runs_new_3_nsl_kdd2025.11.13--13-57-06'  # Partial knowledge model

    print("="*70)
    print("GENERATING SYNTHETIC NSL-KDD SAMPLES")
    print("="*70)
    print(f"Model: {model_path}")
    print(f"Samples: {gen_num}")
    print(f"Device: {device}")
    print("="*70)

    synthetic(model_path, gen_num, device)

    print("\n[OK] Synthetic samples saved!")
    print(f"Location: {model_path}/generated_samples.csv")
    print("="*70)
