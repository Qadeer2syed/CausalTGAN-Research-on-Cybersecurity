...
class DataTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        ...
        ohe = OneHotEncoder()
        # Convert numpy array to DataFrame as required by new rdt API
        data_df = pd.DataFrame({column_name: raw_column_data})
        ohe.fit(data_df, column_name)
    ...
    def _transform_discrete(self, column_transform_info, raw_data):
        ohe = column_transform_info.transform
        column_name = column_transform_info.column_name
        # Pass DataFrame directly to new rdt API
        transformed = ohe.transform(raw_data)
        # Get the transformed columns (one-hot encoded)
        ohe_cols = [col for col in transformed.columns if col.startswith(column_name)]
        return [transformed[ohe_cols].values]
    ...
    def transform(self, raw_data):
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            if column_transform_info.column_type == "continuous":
                ...
                # Pass DataFrame directly for discrete columns
                column_data_list += self._transform_discrete(
                    column_transform_info, raw_data)
    ...
    def _inverse_transform_discrete(self, column_transform_info, column_data):
        ...
        column_name = column_transform_info.column_name
        # Convert numpy array to DataFrame as required by new rdt API
        # The column_data is one-hot encoded, so we need to create a DataFrame with the proper column names
        num_categories = column_data.shape[1]
        column_names_ohe = [f'{column_name}.value{i}' for i in range(num_categories)]
        data_df = pd.DataFrame(column_data, columns=column_names_ohe)
        result = ohe.reverse_transform(data_df)
