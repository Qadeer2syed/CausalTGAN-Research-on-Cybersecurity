...
from dataset import DataTransformer, GeneralTransformer, PlainTransformer
from helper.constant import NSL_KDD_CATEGORY
...
def path_by_name(data_name):
    # All datasets are in real_world folder
...
def get_discrete_cols(data, data_name):
    ...
    # For NSL-KDD, use the predefined categorical features
    if data_name == 'nsl_kdd':
        discrete_cols = NSL_KDD_CATEGORY
    else:
        # Auto-detect categorical columns for other datasets
...
def load_data_graph(data_name, graph_path=None):
    ...
    # For NSL-KDD, use train_full.csv (all 42 features) with causal_graph (10 CRFS features)
    # This mismatch triggers PARTIAL KNOWLEDGE (hybrid) mode:
    #   - Causal generator: 10 CRFS features
    #   - Conditional GAN: Remaining 32 features
    if data_name == 'nsl_kdd':
